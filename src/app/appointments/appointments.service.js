...
angular.module('appointments')
  .service('appointmentService', function (
    ...
    self.msgFields = function (type, appointment) {
      var fields = {
        approval: {
          last_name: appointment.visitor.name ? appointment.visitor.name.split(' ')[0] : appointment.visitor.last_name,
          first_name: appointment.visitor.name ? appointment.visitor.name.split(' ')[1] : appointment.visitor.first_name,
          visitor_phone: appointment.visitor.phone,
          host_last_name: appointment.host.name.split(' ')[0],
          host_first_name: appointment.host.name.split(' ')[1],
          date: $filter('date')(appointment.start_date, 'mediumDate'),
          start_time: $filter('date')(appointment.start_time, 'HH:mm')
        },
        created: {
          last_name: appointment.host.name.split(' ')[0],
          first_name: appointment.host.name.split(' ')[1],
          visitor_last_name: appointment.visitor.name ? appointment.visitor.name.split(' ')[0] : appointment.visitor.last_name,
          visitor_first_name: appointment.visitor.name ? appointment.visitor.name.split(' ')[1] : appointment.visitor.first_name,
          visitor_phone: appointment.visitor.phone,
        }
      };
