...
angular.module('appointments')
  .controller('AFormCtrl', function (
    ...
    vm.viewModel.end_time = moment().add(60, 'm').toDate();
          ...
          return userService.all({q: query, 'only-fields': 'first_name,last_name,phone'})
            .then(function (response) {
              return response.results.map(function (row) {
                return {name: [row.last_name, row.first_name].join(' '), _id: row._id, email: row.email};
              });
            })
            .catch(function (reason) {
              console.log(reason);
      ...
      visitor: {
        get: function (query) {
          return visitorService.all({q: query, 'only-fields': 'first_name,last_name,phone'})
            .then(function (response) {
              return response.results.map(function (row) {
                return {name: [row.last_name, row.first_name].join(' '), _id: row._id, email: row.email, phone: row.phone};
              });
            })
            .catch(function (reason) {
              console.log(reason);
